...
class Device {
	...
	public function isAndroidOrIphone(){
		$ret = false;
		preg_match('/android|iphone/i', $_SERVER['HTTP_USER_AGENT'], $agent);
		if(isset($agent[0])) $ret = strtolower($agent[0]);
		return $ret;
	}
    ...
    public function __call($name, $arguments)
    {
		...
		if(in_array($name, array('group', 'landing', 'new_sharing', 'profile', 'news', 'topic', 'activity', 'sharing'))) {
			$this->display($name);
			exit;
		}
        $key = substr($name, 2);
